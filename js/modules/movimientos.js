                ...
                <form id="formMovimientoInventario" onsubmit="procesarMovimientoInventario(event, '${tipoMovimiento}')">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 15px; margin-bottom: 15px;">
                        ...
                        <div>
                            ...
                            <input type="text" id="movSku" required placeholder="Ej. SKU1001" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none;">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="movMotivo" required placeholder="${esEntrada ? 'Compra a proveedor / Ajuste' : 'Venta directa / Muestrario'}" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.95rem; outline: none;">
                        </div>
...
async function cargarKardexCompleto() {
    ...
    try {
        ...
        if (res && res.status === "success" && res.data && res.data.length > 0) {
            let html = "";
            res.data.forEach(k => {
                let tipoRaw = String(k.Tipo || k.tipo || "").trim().toUpperCase();
                let esEntrada = tipoRaw === 'ENTRADAS' || tipoRaw === 'ENTRADA';
